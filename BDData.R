####################################################################################################
#
# R code to reproduce data download, cleaning and analysis for Bangladesh DHS data for my MSc project
...
downloads <- get_datasets(datasets[grep("BD", datasets$FileName), ])

#Read in data for IR files
bdir94 <- readRDS(downloads$BDIR31FL)
bdir97 <- readRDS(downloads$BDIR3AFL)
bdir04 <- readRDS(downloads$BDIR4JFL)
bdir07 <- readRDS(downloads$BDIR51FL)
bdir11 <- readRDS(downloads$BDIR61FL)
bdir14 <- readRDS(downloads$BDIR72FL)
...
library(plyr)
allsurvs <- rbind.fill(bdir04, bdir07, bdir11, bdir14)
...
#Max CEB is 21, therefore keep all birth order columns
...
## Duplicate row names prevent reshaping ##
#Check for duplicate cases
dupes <- duplicated(mums[,2:5])
count(dupes)

#This shows there are 1823 duplicates

#Drop duplicates on v000:v003
library(dplyr)
mums <- mums %>%
  distinct(v000, v001, v002, v003, .keep_all = TRUE)
