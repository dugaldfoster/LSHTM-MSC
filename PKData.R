####################################################################################################
#
# R code to reproduce data download, cleaning and analysis for Pakistan DHS data for my MSc project
...
downloads <- get_datasets(datasets[grep("PK", datasets$FileName), ])

#Read in data for IR files
pkir91 <- readRDS(downloads$PKIR21FL)
pkir07 <- readRDS(downloads$PKIR52FL)
pkir13 <- readRDS(downloads$PKIR61FL)
...
library(plyr)
allsurvs <- rbind.fill(pkir07, pkir13)
...
#Max CEB is 19, keep all birth order columns (ignore _20 columns)
